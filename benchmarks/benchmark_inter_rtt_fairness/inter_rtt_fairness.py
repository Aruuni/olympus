...
def _trial_key(row: dict):
    """Return the ``(bw, flow2_rtt, run)`` key for a metrics row, or None."""
...
def _run_approach(base_cfg: dict, approach: dict, bench: dict,
    ...
    if rows_by_key:
        print(f'[inter_rtt_bench] {label} resuming '
    ...
    for cell_idx, cell in enumerate(_grid_cells(bench), start=1):
        ...
        for run_number in range(1, int(bench['runs_per_cell']) + 1):
            if (bw_mbps, flow2_rtt_ms, run_number) not in missing_set:
                continue
...
def _ensure_goodput_ratio_metrics(row: dict, force: bool = False) -> None:
    samples = _finite_float(row.get('goodput_ratio_total_samples'))
    if (not force
            and str(row.get('goodput_source', '')).strip() == GOODPUT_SOURCE
...
def _summary_rows(rows: list, bench: dict, recompute_metrics: bool = False) -> list:
    completed = [r for r in rows if not str(r.get('error', '')).strip()]
    for row in completed:
        _ensure_goodput_ratio_metrics(row, force=recompute_metrics)
    fixed_rtt = float(bench.get('fixed_flow_rtt_ms', 20.0))
    out = []
...
def _write_approach_outputs(output_root: str, bench: dict, rows: list,
                            recompute_metrics: bool = False) -> int:
    written = 0
...
def _replot_all(output_root: str, cfg: dict, bench: dict,
                replot_individual: bool = True, selected: set = None,
                recompute_metrics: bool = False) -> None:
    ...
    summary_rows = _summary_rows(all_rows, bench, recompute_metrics=recompute_metrics)
...
def main() -> None:
    ...
    ap.add_argument('--recompute-metrics', action='store_true',
                    help='recompute goodput-ratio metrics from the stored per-run '
                         'flows instead of reusing the values cached in metrics.csv '
                         '(use after a goodput-alignment fix to refresh heatmaps)')
    ...
    if args.plot_only or args.heatmaps_only:
    ...
    try:
        for approach in approaches:
            ...
            if kind not in ('kernel', 'astraea', 'orca') and (
                ...
                raise SystemExit(
                    f'[inter_rtt_bench] approach config for '
            ...
            if approach_config_path:
                print(
                    f'[inter_rtt_bench] {approach["_label"]} '
                    f'config={approach_config_path}',
                    flush=True)
            try:
                rows = _run_approach(
    ...
    if failed_trials:
        raise SystemExit(
            f'[inter_rtt_bench] {failed_trials} trial(s) failed')
