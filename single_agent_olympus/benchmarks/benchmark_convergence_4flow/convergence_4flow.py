...
def main() -> None:
    ...
    print(f'[conv4] base_config={base_config_path}', flush=True)
    ...
    if 'listener' not in paths or 'py' not in paths:
        raise SystemExit('[conv4] base config must define paths.listener and paths.py')
    listener_bin = _resolve_repo_path(paths['listener'])
    python_bin = _resolve_repo_path(paths['py'])

    try:
        for approach in approaches:
            _run_approach(base_cfg, approach, bench, output_root,
