...
def _plot_fairness_run(receiver_flows: list, flow_plan: list, bench: dict,
                       label: str, output: str, rl_flows: list = None) -> str:
    ...
    axes[2].set_ylabel('RTT estimate ms')
    axes[2].set_ylim(bottom=0)
    axes[2].set_title('Per-flow SRTT and Kalman min RTT from RL clients',
                      fontsize=9, loc='left')
    if srtt_plotted or kalman_plotted:
        axes[2].legend(loc='upper right', ncol=3, fontsize=7, frameon=False)
    else:
        axes[2].text(0.5, 0.5, 'No RL-client SRTT/Kalman log for this approach',
...
def main() -> None:
    ...
    print(f'[fair_bench] base_config={base_config_path}', flush=True)
    ...
    if 'listener' not in paths or 'py' not in paths:
        raise SystemExit('[fair_bench] base config must define paths.listener and paths.py')
