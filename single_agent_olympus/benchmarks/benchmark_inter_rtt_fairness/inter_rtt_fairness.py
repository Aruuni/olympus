#!/usr/bin/env python3
"""Inter-RTT fairness heatmap benchmark for two-flow sharing.

Grid:
  BW       = bw_mbps_values (Mbps)
  Flow-2 RTT = rtt_ms_values (ms)

Flow 1 has a FIXED RTT (fixed_flow_rtt_ms, default 20 ms) and runs for the
full episode. Flow 2 has the VARYING RTT given by the grid row and joins at
the configured absolute start time. Each flow's full one-way RTT is applied
as netem on its own sender access link (flow 1 on c1 -> s1, flow 2 on
c2 -> s1); the shared s1-s2 link carries no propagation delay. The smaller
RTT is still used only to size the shared bottleneck BDP buffer, so buffer
behaviour is unchanged -- only where the delay sits moves to the access
links.

Metrics are scored from the configured final window (goodput ratio).
"""
...
FAIRNESS_FIELDS = [
    ...
    'cell', 'run', 'seed', 'duration_s', 'bw_mbps', 'rtt_ms',
    'fixed_flow_rtt_ms', 'flow2_rtt_ms', 'base_delay_ms', 'flows',
...
SUMMARY_FIELDS = [
    'approach', 'data_folder', 'plot_label', 'kind', 'algorithm', 'kernel_cc',
    'state', 'reward', 'training_environment', 'benchmark_environment',
    'bw_mbps', 'rtt_ms', 'fixed_flow_rtt_ms', 'runs_completed', 'runs_expected',
    'goodput_ratio_total_mean', 'goodput_ratio_total_std',
...
def _inter_rtt_cfg(cfg: dict) -> dict:
    bench = dict(cfg.get('benchmark') or {})
    bench.setdefault('name', 'inter_rtt_fairness_two_flow_join_halfway')
    ...
    bench.setdefault('fixed_flow_rtt_ms', 20)
    bench.setdefault('bw_mbps_values', [20, 50, 100, 150, 200])
    bench.setdefault('rtt_ms_values', [20, 30, 40, 50, 60, 70])
...
def _rtt_split(fixed_flow_rtt_ms: float, flow2_rtt_ms: float) -> tuple:
    """Put each flow's FULL one-way RTT on its own sender access link.

    Flow 1's RTT is applied as netem on the c1 -> s1 link and flow 2's RTT
    on the c2 -> s1 link; the shared s1-s2 link carries no propagation delay
    (MininetEnv suppresses it whenever per_flow_delays is set). The returned
    base delay is the smaller RTT and is used ONLY to size the shared
    bottleneck BDP buffer (qsize), exactly as before -- so per-flow RTTs and
    buffer behaviour are unchanged; only the physical location of the delay
    moves onto the access links.

    Returns (buffer_sizing_rtt_ms, [flow1_rtt_ms, flow2_rtt_ms]).
    """
    f1 = float(fixed_flow_rtt_ms)
    f2 = float(flow2_rtt_ms)
...
def _prepare_fairness_runtime_cfg(base_cfg: dict, approach: dict,
    ...
    scratch_dir = os.path.join(
        '/tmp', f'olympus_inter_rtt_runtime_{os.getpid()}', approach['_label'])
...
def _grid_cells(bench: dict) -> list:
    """Grid rows are flow-2 RTT values; flow 1 stays at fixed_flow_rtt_ms."""
    cells = []
...
def _static_schedule(bench: dict, bw_mbps: float, base_delay_ms: float,
                     seed: int) -> dict:
    return {
        'seed': int(seed),
        'initial': {'bw': float(bw_mbps), 'delay': float(base_delay_ms)},
        'changes': [],
        'rows': [{'t': 0, 'bw': float(bw_mbps), 'delay': float(base_delay_ms)}],
    }
...
def _iperf_rtt_flows(run_dir: str, flow_plan: list) -> list:
    """Per-flow TCP RTT measured by iperf3 (from the sender/client JSON).

    Works for every approach including kernel/astraea, which have no
    RL-client state logs. Interval timestamps are shifted by the flow's
    scheduled start so the RTT trace aligns with the goodput panels."""
    out = []
    ...
    if not any(f['t'].size for f in out):
        # No iperf RTT (Orca): fall back to the ss-sidecar srtt traces,
        # which are returned in this exact shape.
...
def _state_flows(run_dir: str, alg_name: str, flow_plan: list) -> list:
    pattern = os.path.join(run_dir, f'{alg_name}_state_ep000001_flow*.csv')
...
def _state_flows_from_paths(paths: list, flow_plan: list) -> list:
    ...
    for path in paths:
        m = re.search(r'_flow(\d+)\.csv$', path)
        flow = _flow_id_from_suffix(int(m.group(1)), min_suffix) if m else len(out) + 1
...
def _plot_fairness_run(receiver_flows: list, flow_plan: list, bench: dict,
    ...
    fig, axes = plt.subplots(7, 1, figsize=(11.5, 16.0), sharex=True)
    ...
    iperf_plotted = _plot_flow_lines(
        ...
        label_prefix='iperf RTT flow', linewidth=1.3)
    f1_rtt = float(bench.get('fixed_flow_rtt_ms', bench.get('rtt_ms', 20.0)))
    f2_rtt = float(bench.get('flow2_rtt_ms', bench.get('rtt_ms', 20.0)))
    axes[2].axhline(f1_rtt, color='black', linestyle='--',
                    linewidth=1.0, label=f'flow1 RTT {f1_rtt:g}ms')
    axes[2].axhline(f2_rtt, color='black', linestyle=':',
                    linewidth=1.0, label=f'flow2 RTT {f2_rtt:g}ms')
    axes[2].set_ylabel('RTT ms')
    axes[2].set_ylim(bottom=0)
    axes[2].set_title('Per-flow iperf3 RTT (and RL SRTT/Kalman when available)',
                      fontsize=9, loc='left')
    if srtt_plotted or kalman_plotted or iperf_plotted:
        axes[2].legend(loc='upper right', ncol=3, fontsize=7, frameon=False)
    else:
        axes[2].text(0.5, 0.5, 'No iperf3 RTT or RL SRTT/Kalman log for this run',
                     transform=axes[2].transAxes, ha='center', va='center',
                     fontsize=9, color='#555555')

    cwnd_plotted = _plot_flow_lines(axes[3], rl_flows, 'cwnd', colors, linewidth=0.85)
    ...
    if cwnd_plotted:
        ...
        axes[3].text(0.5, 0.5, 'No RL-client CWND log for this approach',
    ...
    axes[4].axhline(1.0, color='black', linestyle='--', linewidth=0.8, alpha=0.7)
    ...
    if mult_plotted or reward_plotted:
        ...
        axes[4].legend(handles + r_handles, labels + r_labels,
                       loc='upper right', ncol=2, fontsize=7, frameon=False)
    else:
        axes[4].text(0.5, 0.5, 'No RL action/reward log for this approach',
                     transform=axes[4].transAxes, ha='center', va='center',
                     fontsize=9, color='#555555')

    axes[5].plot(t, series['total'], color='#59a14f', linewidth=1.5,
                 label='aggregate goodput')
    axes[5].axhline(float(bench['bw_mbps']), color='black', linestyle='--',
                    linewidth=1.0, label='link capacity')
    axes[5].set_ylabel('Total Mbps')
    axes[5].set_ylim(bottom=0)
    axes[5].legend(loc='upper right', fontsize=8, frameon=False)

    axes[6].plot(t, series['jain'], color='#e15759', linewidth=1.4,
                 label='Jain fairness')
    axes[6].set_ylabel('Jain fairness')
    axes[6].set_xlabel('Time (s)')
    axes[6].set_ylim(0, 1.05)
    axes[6].legend(loc='lower right', fontsize=8, frameon=False)
    axes[6].set_xlim(0, float(bench['duration_s']))
...
def _row(approach: dict, bench: dict, cell_idx: int, run_number: int,
         bw_mbps: float, flow2_rtt_ms: float, base_delay_ms: float,
         seed: int, run_dir: str, schedule_csv: str, flow_schedule_csv: str,
         checkpoint: str, state_logs: list, individual_plot: str,
         metrics: dict, error: str = '') -> dict:
    fixed_rtt = float(bench.get('fixed_flow_rtt_ms', 20.0))
    row = {
        ...
        'rtt_ms': float(flow2_rtt_ms),
        'fixed_flow_rtt_ms': fixed_rtt,
        'flow2_rtt_ms': float(flow2_rtt_ms),
...
def _run_kernel_trial(instance_id: int, approach: dict, bench: dict,
                      ...
                      flow2_rtt_ms: float, seed: int, run_dir: str,
                      schedule_csv: str, flow_schedule_csv: str) -> dict:
    flow_plan = _flow_plan(bench)
    fixed_rtt = float(bench.get('fixed_flow_rtt_ms', 20.0))
    base_delay, per_flow_delays = _rtt_split(fixed_rtt, flow2_rtt_ms)
    ...
    metrics = {}
    try:
        print(f'[inter_rtt_bench] slot={instance_id} {approach["_label"]} '
              f'bw={bw_mbps:g} f1_rtt={fixed_rtt:g} f2_rtt={flow2_rtt_ms:g} '
              f'run={run_number} -C {cc_algo}', flush=True)
        env = MininetEnv(
            n=2,
            bw=float(bw_mbps),
        ...
        try:
            _plot_fairness_run(
                ...
                {'duration_s': int(bench['duration_s']), 'bw_mbps': float(bw_mbps),
                 'fixed_flow_rtt_ms': fixed_rtt, 'flow2_rtt_ms': float(flow2_rtt_ms),
                 'score_window_s': float(bench.get('score_window_s', 20.0))},
                f'{_approach_plot_label(approach)} BW={bw_mbps:g} '
                f'f1RTT={fixed_rtt:g} f2RTT={flow2_rtt_ms:g} run={run_number}',
                plot_path,
                iperf_rtt_flows=_iperf_rtt_flows(run_dir, flow_plan),
            )
        except Exception as plot_exc:
            print(f'[inter_rtt_bench] individual plot failed for {plot_path}: {plot_exc}',
                  flush=True)

    return _row(
        approach, bench, cell_idx, run_number, bw_mbps, flow2_rtt_ms,
        base_delay, seed, run_dir, schedule_csv, flow_schedule_csv, '', [],
...
def _run_orca_trial(instance_id: int, approach: dict, bench: dict,
                    ...
                    flow2_rtt_ms: float, seed: int, run_dir: str,
                    schedule_csv: str, flow_schedule_csv: str) -> dict:
    """Real Orca (SIGCOMM'20) inter-RTT fairness trial: flow 1 fixed RTT,
    flow 2 varying RTT (per-flow access-link netem), driven by
    orca-server-mahimahi/clientThr. Receiver goodput is parsed into
    csvs/x{f}.csv so metrics/plots/completion are unchanged. clientThr does
    not report RTT, so the per-flow RTT plot panel is empty for Orca."""
    flow_plan = _flow_plan(bench)
    fixed_rtt = float(bench.get('fixed_flow_rtt_ms', 20.0))
    base_delay, per_flow_delays = _rtt_split(fixed_rtt, flow2_rtt_ms)
    ...
    try:
        print(f'[inter_rtt_bench] slot={instance_id} {approach["_label"]} '
              f'bw={bw_mbps:g} f1_rtt={fixed_rtt:g} f2_rtt={flow2_rtt_ms:g} '
              f'run={run_number} (real-orca)', flush=True)
        env = MininetEnv(
            n=2,
            bw=float(bw_mbps),
            delay=float(base_delay),
...
def _run_model_trial(instance_id: int, approach: dict, bench: dict,
                     ...
                     bw_mbps: float, flow2_rtt_ms: float, seed: int,
    ...
    cfg['listener_single_flow'] = False
    ...
    ecfg = {
        'bw': float(bw_mbps),
        'delay': float(base_delay),
        'per_flow_delays': [float(d) for d in per_flow_delays],
        'bdp_mult': float(bench['bdp_mult']),
        'flows': 2,
        'duration': int(bench['duration_s']),
        'start_delays': [p['start'] for p in flow_plan],
    ...
    try:
        print(f'[inter_rtt_bench] slot={instance_id} {approach["_label"]} '
              f'bw={bw_mbps:g} f1_rtt={fixed_rtt:g} f2_rtt={flow2_rtt_ms:g} '
              f'run={run_number}', flush=True)
        run_episode(cfg, ecfg, 1, listener_bin, python_bin, '', '', instance_id)
        parse_error = _copy_receiver_iperf_outputs(cport, run_dir, flow_plan, bench)
        flows = _receiver_iperf_flows(run_dir, flow_plan)
    ...
    state_logs = sorted(glob.glob(os.path.join(
        run_dir, f'{approach["algorithm"]}_state_ep000001_flow*.csv')))
    plot_path = os.path.join(run_dir, 'fairness_run.pdf')
    if flows and _as_bool(bench.get('plot_episodes'), default=True):
        try:
            _plot_fairness_run(
    ...
    return _row(
        approach, bench, cell_idx, run_number, bw_mbps, flow2_rtt_ms,
        base_delay, seed, run_dir, schedule_csv, flow_schedule_csv,
        checkpoint, state_logs, plot_path, metrics, error)
...
def _approach_slot(instance_id: int, work_q, result_q, approach: dict,
    ...
    while True:
        ...
        if item is None:
            break
        (cell_idx, run_number, bw_mbps, flow2_rtt_ms, seed, run_dir,
        ...
        if kind in ('kernel', 'astraea'):
            row = _run_kernel_trial(
                ...
                bw_mbps, flow2_rtt_ms, seed, run_dir, schedule_csv,
                flow_schedule_csv)
        elif kind == 'orca':
            row = _run_orca_trial(
                instance_id, approach, bench, cell_idx, run_number,
...
def _run_approach(base_cfg: dict, approach: dict, bench: dict,
    ...
    if kind not in ('kernel', 'astraea', 'orca'):
        ...
        if not os.path.exists(checkpoint):
            raise SystemExit(f'[inter_rtt_bench] checkpoint not found for {label}: {checkpoint}')
    ...
    for cell_idx, cell in enumerate(_grid_cells(bench), start=1):
        ...
        flow2_rtt_ms = float(cell['rtt'])
        base_delay, _ = _rtt_split(fixed_rtt, flow2_rtt_ms)
        cell_dir = os.path.join(approach_dir, _cell_name(bw_mbps, flow2_rtt_ms))
        for run_number in range(1, int(bench['runs_per_cell']) + 1):
            ...
            schedule = _static_schedule(bench, bw_mbps, base_delay, seed)
    ...
    total = _expected_trial_count(bench)
    ...
    try:
        while completed < total:
            ...
            print(f'[inter_rtt_bench] {label} cell={row["cell"]} '
                  f'bw={float(row["bw_mbps"]):g} f2_rtt={float(row["rtt_ms"]):g} '
...
def _write_summary_json(summary_rows: list, output_json: str) -> None:
    ...
    for row in summary_rows:
        ...
        payload[app][cell] = {
            'bw_mbps': row['bw_mbps'],
            'rtt_ms': row['rtt_ms'],
            'fixed_flow_rtt_ms': row.get('fixed_flow_rtt_ms', ''),
...
def _plot_heatmaps(summary_rows: list, bench: dict, output_pdf: str) -> None:
    if not summary_rows:
        print('[inter_rtt_bench] no summary rows to plot', flush=True)
        return
    fixed_rtt = float(bench.get('fixed_flow_rtt_ms', 20.0))
    ...
    with PdfPages(tmp_path) as pdf:
        for approach in approaches:
            ...
            ax.set_ylabel('Flow-2 RTT (ms)')
            ax.set_title(f'Goodput Ratio (flow 1 fixed at {fixed_rtt:g} ms RTT)')
    ...
    print(f'[inter_rtt_bench] heatmap -> {output_pdf}', flush=True)
...
def _write_approach_outputs(output_root: str, bench: dict, rows: list) -> int:
    ...
    for approach in sorted({r.get('approach', '') for r in rows if r.get('approach')}):
        ...
        try:
            ...
            print(f'[inter_rtt_bench] approach summary skipped for {approach}: {exc}',
                  flush=True)
    return written
...
def _replot_individual_rows(rows: list, bench: dict = None) -> int:
    ...
    for row in rows:
        ...
        flow2_rtt = float(row.get('flow2_rtt_ms', row.get('rtt_ms', 50)) or 50)
        row_fixed_rtt = float(row.get('fixed_flow_rtt_ms', fixed_rtt) or fixed_rtt)
        score_window = float(row.get('score_window_s', 20) or 20)
        flow_plan = _flow_plan_from_row(row)
        rl_flows = _rl_flow_traces_for_row(row, flow_plan)
        iperf_rtt_flows = _iperf_rtt_flows(str(row.get('run_dir', '')), flow_plan)
        ...
        label = (
            ...
            f'BW={bw:g} f1RTT={row_fixed_rtt:g} f2RTT={flow2_rtt:g} '
            f'run={row.get("run", "")}'
        )
        ...
        plot_bench.update({
            'duration_s': duration,
            'bw_mbps': bw,
            'fixed_flow_rtt_ms': row_fixed_rtt,
            'flow2_rtt_ms': flow2_rtt,
            'score_window_s': score_window,
        })
        try:
            _plot_fairness_run(
                flows, flow_plan, plot_bench, label, output,
                rl_flows=rl_flows, iperf_rtt_flows=iperf_rtt_flows)
            row['individual_plot'] = output
            plotted += 1
        except OSError as exc:
            print(f'[inter_rtt_bench] individual plot skipped for {output}: {exc}',
                  flush=True)
    return plotted
...
def _replot_all(output_root: str, cfg: dict, bench: dict,
    ...
    if selected and not rows:
        raise SystemExit('[inter_rtt_bench] no existing rows matched requested approach: '
    ...
    try:
        ...
        print(f'[inter_rtt_bench] root summary skipped: {exc}', flush=True)
    _restore_sudo_user_ownership(output_root)
    print(f'[inter_rtt_bench] replotted rows={len(rows)} total_rows={len(all_rows)} '
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description='Run the two-flow inter-RTT fairness heatmap benchmark.')
    ...
    bench = _inter_rtt_cfg(cfg)

    base_config = cfg.get('base_config', '../../config.yaml')
    ...
    if not configured_approaches:
        ...
        if selected:
            ...
            raise SystemExit(
                '[inter_rtt_bench] requested approach is not listed in benchmark config'
                + (f': {missing}' if missing else '')
                + (f'; available: {", ".join(available)}' if available else '')
            )
        raise SystemExit('[inter_rtt_bench] no approaches listed in benchmark config'
                         + (f'; available: {", ".join(available)}' if available else ''))

    approaches, skipped = _split_by_completion(configured_approaches, output_root, bench)
    print(f'[inter_rtt_bench] base_config={base_config_path}', flush=True)
    print(f'[inter_rtt_bench] output_root={output_root}', flush=True)
    print(f'[inter_rtt_bench] benchmark={bench["name"]} duration={bench["duration_s"]}s '
          f'flow1_start={bench.get("first_flow_start_s", 0)}s '
          f'flow2_start={bench["second_flow_start_s"]}s '
          f'fixed_flow_rtt={bench.get("fixed_flow_rtt_ms", 20)}ms '
          f'score_window={bench.get("score_window_s", 20)}s', flush=True)
    print('[inter_rtt_bench] bw_grid='
          + ','.join(str(v) for v in bench['bw_mbps_values']), flush=True)
    print('[inter_rtt_bench] flow2_rtt_grid='
          + ','.join(str(v) for v in bench['rtt_ms_values']), flush=True)
    print(f'[inter_rtt_bench] runs_per_cell={bench["runs_per_cell"]} '
          f'n_parallel={bench.get("n_parallel", 1)}', flush=True)
    if skipped:
        print('[inter_rtt_bench] skipping complete data='
              + ', '.join(a['_label'] for a in skipped), flush=True)
    print('[inter_rtt_bench] approaches_to_run='
          + (', '.join(a['_label'] for a in approaches) if approaches else '(none)'),
          flush=True)

    if args.dry_run:
        for approach in skipped:
            print(f'[inter_rtt_bench] dry-run would skip complete data: '
                  f'{os.path.join(output_root, approach["_label"])}', flush=True)
        for approach in approaches:
            print(f'[inter_rtt_bench] dry-run would run: '
                  f'{os.path.join(output_root, approach["_label"])} '
                  f'trials={_expected_trial_count(bench)}', flush=True)
        return

    if not approaches:
        print('[inter_rtt_bench] no approaches need running; rebuilding plots from existing data',
              flush=True)
        _replot_all(output_root, cfg, bench)
        return
    ...
    if 'listener' not in paths or 'py' not in paths:
        raise SystemExit('[inter_rtt_bench] base config must define paths.listener and paths.py')
    ...
    _replot_all(output_root, cfg, bench)
    print(f'[inter_rtt_bench] done metrics={os.path.join(output_root, "metrics.csv")}',
          flush=True)
