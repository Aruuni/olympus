"""Per-flow rollout worker for recurrent TD3."""
...
import tcp_sockopt
from single_agent_olympus.algorithms.td3 import model
...
def run():
    ...
    traj_id = f'td3_{cport}_{episode}_{flow_id}'
    ...
    # BBR3-style independent probe; toggle via SAO_BBR_PROBE_ENABLED=1
    # (or agent.bbr_probe.enabled: true in config). Works with any reward.
    probe = BbrProbe.from_env()

    role = 'lagged-bg' if is_lagged_policy else 'train'
    print(f'[worker alg=td3 reward={reward_name} cport={cport} flow={flow_id} '
          f'role={role}] '
    ...
    try:
        while True:
            ...
            desired_cwnd = int(np.clip(round(cur_cwnd * mult), cwnd_min, cwnd_max))

            # Feed kernel-smoothed srtt into the probe's min-RTT filter
            # (srtt_us is usec<<3; fall back to avg_urtt pre-handshake).
