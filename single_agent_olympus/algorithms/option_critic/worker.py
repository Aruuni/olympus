...
Environment variables (set by orchestrator → oc_listener → inherited here):
  ...
  Reward settings are provided by the shared single_agent_olympus reward module.
  ...
  OC_MULT_MIN         min cwnd multiplier (default 0.5)
  OC_MULT_MAX         max cwnd multiplier (default 2.0)
...
from single_agent_olympus.common.bbr_probe import BbrProbe
from single_agent_olympus.common.registry import reward_module


def make_reward_calc():
    reward_name = os.environ.get('SAO_REWARD', 'kalman_fit')
...
def run():
    ...
    mult_min    = float(os.environ.get('OC_MULT_MIN', '0.5'))
    mult_max    = float(os.environ.get('OC_MULT_MAX', '2.0'))
    ...
    cwnd_quant_residual = 0.0    # preserves sub-packet action intent across integer CWND writes
    ...
    try:
        while True:
            ...
            desired_cwnd_f = cur_cwnd * mult + cwnd_quant_residual
            quantized_cwnd = round(desired_cwnd_f)
            agent_cwnd  = int(np.clip(quantized_cwnd, cwnd_min, cwnd_max))
            ...
            # Maintain quant residual only when the agent's write actually
            # happened; on probe-locked steps the residual would chase a
            # cwnd value the agent didn't pick.
            if not agent_locked and new_cwnd == quantized_cwnd:
                cwnd_quant_residual = float(np.clip(desired_cwnd_f - new_cwnd, -1.0, 1.0))
            else:
                cwnd_quant_residual = 0.0
