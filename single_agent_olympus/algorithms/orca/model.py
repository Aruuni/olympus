...
* Actor actions are mapped through `int(100 * 4**a)` and applied as a CWND
  percentage, matching the upstream shared-memory protocol.
...
from single_agent_olympus.common.state_plugins import assert_state_compatible
...
class Actor(nn.Module):
    ...
    def act(self, s_np: np.ndarray, noise_std: float = 0.0):
        ...
        action = float(a.item())
        percent = action_to_percent(action)
        return action, percent / 100.0
...
def action_to_percent(action: float) -> int:
    action = max(min(float(action), 1.0), -1.0)
    return int((4.0 ** action) * 100.0)


def action_to_mult(a: torch.Tensor) -> torch.Tensor:
    return torch.pow(torch.full_like(a, 4.0), a)
