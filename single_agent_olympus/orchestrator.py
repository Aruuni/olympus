"""
orchestrator.py — parallel episode manager for single_agent_olympus.

The learner and reward are hot-swappable through config.yaml:
  runtime.algorithm: td3
  runtime.reward:    kalman_fit

Usage:
  sudo -E env PATH="$PATH" HOME="$HOME" \\
    python single_agent_olympus/orchestrator.py \\
...
from single_agent_olympus.common.episode_plot import (
...
from single_agent_olympus.common.registry import (
    learner_script,
    reward_module,
...
def _final_runtime_cleanup(learner_port: int):
    ...
    _pkill('single_agent_olympus/algorithms/.*/worker.py')
    _pkill('single_agent_olympus/algorithms/.*/learner.py')
...
def _activate_runtime_blocks(cfg: dict) -> None:
    ...
    reward_name = runtime.get('reward', 'kalman_fit')
    ...
    cfg['reward'] = dict(cfg.get('reward') or {})
...
def _build_pool(cfg):
    ...
    # top level instead of in single_agent_olympus/environments/*.yaml.
    if 'sweep' in cfg:
...
def run_episode(cfg, ecfg, episode, listener_bin, python_bin,
    ...
    worker_env.update({
        ...
        **_bbr_probe_env(cfg, a_cfg),
        ...
        'OC_MULT_MIN':       str(float(a_cfg.get('mult_min', 0.5))),
        'OC_MULT_MAX':       str(float(a_cfg.get('mult_max', 2.0))),
        ...
        'OC_W_OVERSHOOT':      str(float(r_cfg.get('w_overshoot',       3.0))),
        'OC_OVERSHOOT_CAP':    str(float(r_cfg.get('overshoot_cap',    10.0))),
        'OC_OVERSHOOT_THRESHOLD': str(float(r_cfg.get('overshoot_threshold', 2.0))),
        'OC_W_SRTT_DRIFT':     str(float(r_cfg.get('w_srtt_drift',
                                                   r_cfg.get('w_kalman_drift', 2.0)))),
        'OC_SRTT_DRIFT_CAP':   str(float(r_cfg.get('srtt_drift_cap',
                                                   r_cfg.get('kalman_drift_cap', 4.0)))),
        'OC_W_KALMAN_DRIFT':   str(float(r_cfg.get('w_kalman_drift',
                                                   r_cfg.get('w_srtt_drift', 2.0)))),
        'OC_KALMAN_DRIFT_CAP': str(float(r_cfg.get('kalman_drift_cap',
                                                   r_cfg.get('srtt_drift_cap', 4.0)))),
        'OC_ORCA_DELAY_MARGIN_COEF': str(float(r_cfg.get('delay_margin_coef', 1.25))),
        'OC_W_FAIRNESS':       str(float(r_cfg.get('w_fairness', 2.0))),
        'OC_FAIRNESS_CAP':     str(float(r_cfg.get('fairness_cap', 10.0))),
    ...
    if ecfg.get('per_flow_state_logs'):
        ...
        flow_logs = sorted(
            glob.glob(f'{root}_flow*{ext or ".csv"}'),
...
def _slot_process(instance_id, work_q, result_q,
                  cfg, listener_bin, python_bin, learner_state):
...
def main():
    ...
    print(f'[orch] environment={env_meta.get("name", "config")}  '
    ...
    for i in range(n_parallel):
        p = multiprocessing.Process(
            ...
            args   = (i, work_q, result_q, cfg,
                      listener_bin, python_bin, learner_state),
            daemon = True,
        )
        p.start()
        slot_procs.append(p)
    ...
    def _next():
        ...
        if not pending:
            ...
            if do_shuffle:
                random.shuffle(pending)
        ep   = episode_counter
        ecfg = _materialize_lagged_policy_episode(pending.pop(0), ep)
        episode_counter += 1
        return ep, ecfg

    returns_fields = [
        'episode', 'environment', 'bw', 'delay', 'scheduled',
        'schedule_changes', 'episode_type', 'return',
    ...
    def _generate_returns_plot():
        try:
            from single_agent_olympus.common.plot_returns_watcher import generate_plot
            generate_plot(
