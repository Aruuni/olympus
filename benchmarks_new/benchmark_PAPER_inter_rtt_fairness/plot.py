...
def _write_plot(groups, runs, output):
    fig, axis = plt.subplots(figsize=(3, 1.1))
    if not draw_fairness(axis, groups, runs):
        plt.close(fig)
        return False
    add_run_legend(fig, runs, bbox_y=1.12)
    save_figure(fig, output)
    print(f'[paper_inter_plot] wrote {output}')
    return True
...
def main(argv=None):
    ...
    if not groups:
        print(f'[paper_inter_plot] no per-flow traces beneath {root}')
        return 1
