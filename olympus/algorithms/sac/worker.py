"""Per-flow rollout worker for recurrent SAC.

Mirrors the td3 worker's plumbing (fixed-cadence control loop, BBR probe,
async experience pushing, weight pulls, lagged self-play).  The only RL
difference is exploration: SAC's policy is *intrinsically* stochastic, so the
worker samples the action from the policy instead of adding an external
Gaussian noise schedule.  Set SAO_DETERMINISTIC=1 (or the lagged-policy default)
to act on the distribution mean — the standard SAC evaluation behaviour.
"""
...
from olympus.algorithms.sac import model
...
def _carry_forward_drained_metrics(raw: dict, history: dict) -> None:
    """Hide empty DeepCC polling windows from continual reward/state inputs."""
    for name in ('avg_thr', 'avg_urtt', 'thr_cnt'):
        try:
            valid = float(raw.get(name, 0.0) or 0.0) > 0.0
        except (TypeError, ValueError):
            valid = False
        if valid:
    ...
    count_names = [name for name in ('cnt', 'count') if name in raw]
    valid_count = None
    for name in count_names:
        try:
            if float(raw.get(name, 0.0) or 0.0) > 0.0:
                valid_count = raw[name]
                break
        except (TypeError, ValueError):
            pass
    if valid_count is not None:
        history['cnt'] = valid_count
    elif 'cnt' in history:
        for name in count_names:
...
def run():
    ...
    continual = dict(cfg.get('continual') or {})
    continual.update(dict(((cfg.get('deployment') or {}).get(
        'continual') or {})))
    ...
    # SAC exploration is intrinsic (the policy is stochastic). Deterministic
    # mode acts on the distribution mean — standard SAC evaluation.
    ...
    if ckpt_path and os.path.exists(ckpt_path):
        try:
            ...
            ckpt_hidden, ckpt_head_hidden = model.actor_arch_from_checkpoint(
            ...
            if (ckpt_hidden, ckpt_head_hidden) != (hidden, current_head_hidden):
                print(f'[worker cport={cport}] using checkpoint actor config '
    ...
    next_tick = t0
    traj_id = f'sac_{cport}_{episode}_{flow_id}'
    ...
    print(f'[worker alg=sac reward={reward_name} cport={cport} flow={flow_id} '
          f'role={role}] '
          f'started interval={interval_ms}ms det={int(deterministic)} '
          f'bbr_probe={int(probe.enabled)}',
          flush=True)
    ...
    try:
        while True:
            ...
            if continual_enabled:
                _carry_forward_drained_metrics(raw, drained_history)
            ...
            a, mult, log_std, hid = actor.act(norm_s, hid, deterministic=deterministic)
            ...
            if agent_locked:
                ...
                # iteration's push doesn't pair the agent's pre-probe action
                # with the probe's induced reward / next state.
                prev_state = None
                prev_action = 0.0
            else:
                prev_state = norm_s
