#!/usr/bin/env bash
# All-encompassing runtime reset for olympus.
#
...
echo "[reset] killing olympus processes..."
...
if [[ "${SAO_RESET_SKIP_ORCHESTRATOR:-0}" != "1" ]]; then
  kill_pattern "olympus/orchestrator.py"
fi
kill_pattern "olympus/algorithms/.*/worker.py"
kill_pattern "olympus/algorithms/.*/learner.py"
