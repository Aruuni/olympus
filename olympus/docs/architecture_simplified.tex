...
% ---- one rollout (= instance i): env  ->  worker(state/action/reward) ----
...
% dashed background copies of the env -- one per environment type (x M)
...
% worker shell, with internal state + action + reward module decks
...
% type. The up-right stacking echoes the "x M environment types" pile, and the
% "x M" multiplier calls out that there are as many buffers as environment types.
...
% ---- stack the rollout row into N parallel workers ----
\begin{scope}[on background layer]
...
% "x M different environment backends" -- sits on top of the TCP environment
\node[font=\scriptsize\itshape,anchor=south west,inner sep=1pt]
      at ([xshift=3mm,yshift=1.5mm]rollout.north -| env)
      {$\times\,M$ environment backends};

% ---- extra worker agents — present ONLY in the multi-agent case ----
\begin{scope}[on background layer]
...
% the control loop lives between the worker and the environment's tcp flow(s).
...
% setsockopt(cwnd): worker action deck -> tcp flow   (upper line)
...
% getsockopt(tcp_info): tcp flow -> worker state deck   (lower line)
...
% structural 1:1 assignment: each tcp flow is driven by exactly one worker
\draw[<->,thin,densely dashed,draw=black!55]
      (tcpflow.east) -- node[lbl,align=center]{one worker\\per flow}
      (worker.west |- tcpflow);
