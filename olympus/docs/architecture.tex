...
% architecture_new.tex — shorter variant of architecture.tex, with the
% getsockopt / setsockopt data lines drawn as right-angle (orthogonal) arrows
% (each makes two 90-degree turns) instead of arcs; getsockopt routed below
% setsockopt.
% Compile standalone:  pdflatex architecture_new.tex
...
\node[bridge,right=of env] (listener)
     {oc\_bridge \\\scriptsize \texttt{ss} match \texttt{port=$P_i$}\\\scriptsize duplicate fd, hold env var};

% worker shell, with internal state + action module decks
\node[agent,right=26mm of listener,minimum width=64mm,minimum height=17mm] (worker) {};
...
\node[proc,above=12mm of worker,fill=blue!13,minimum width=46mm,minimum height=17mm] (learner) {};
...
\node[font=\scriptsize\itshape,inner sep=1pt]
      at ([yshift=4mm]rollout.north -| listener)
...
% sides -- so they never cross or overlap. Both enter the env towards its left.
\coordinate (setline) at ([yshift=-3mm]rollout.south);
\coordinate (getline) at ([yshift=-7.5mm]rollout.south);

% setsockopt(cwnd): worker action deck -> env   (upper line)
\draw[data,->] (waction.south) -- (waction.south |- setline)
      -- node[lbl,below=0.2mm]{setsockopt(cwnd) $\downarrow$}
         ([xshift=-3mm]env.south |- setline)
      -- ([xshift=-3mm]env.south);

% getsockopt(tcp_info): env -> worker state deck   (lower line)
\draw[data,->] ([xshift=-9mm]env.south) -- ([xshift=-9mm]env.south |- getline)
      -- node[lbl,below=0.2mm]{getsockopt(tcp\_info) $\uparrow$}
